...
export function useStore() {
  ...
  useEffect(() => {
    ...
    // ── 團主驗證碼（非阻塞載入）──────────────────────────────────
    unsubs.push(onSnapshot(doc(db, 'config', 'leaderCode'), (snap) => {
  ...
  async function setLeaderCode(code) {
    await setDoc(doc(db, 'config', 'leaderCode'), { code });
  }
