...
function SessionCard({ session, sessionOrders, onOrder, onExtend, onClose, onReset, onContinue, isLeader }) {
  ...
  return (
    <div className={`bg-white rounded-xl shadow p-4 space-y-3 ${isExpired ? 'opacity-70' : ''}`}>
      ...
      {isStale && isLeader && (
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
          ...
          <div className="flex gap-2">
            <button onClick={() => onContinue(session.id)} className="flex-1 text-sm bg-amber-500 text-white py-1.5 rounded-lg font-medium hover:bg-amber-600">繼續此單</button>
            <button onClick={() => setConfirmReset(true)} className="flex-1 text-sm bg-gray-100 text-gray-600 py-1.5 rounded-lg font-medium hover:bg-gray-200">刪除</button>
          </div>
        </div>
      )}
      ...
      {isLeader && !isStale && (
        <div className="flex gap-2 flex-wrap">
          {!isExpired && (
            <button onClick={() => onExtend(session.id, 15)} className="text-xs border border-blue-300 text-blue-500 px-2 py-1 rounded hover:bg-blue-50">+15 分鐘</button>
          )}
          <button onClick={() => setConfirmClose(true)} className="text-xs border border-gray-300 text-gray-500 px-2 py-1 rounded hover:bg-gray-50">關閉</button>
          <button onClick={() => setConfirmReset(true)} className="text-xs border border-red-200 text-red-400 px-2 py-1 rounded hover:bg-red-50">重置</button>
        </div>
      )}
      ...
      {!isLeader && (
        canOrder ? (
          <button onClick={() => onOrder(session.id)} className="w-full bg-orange-500 text-white py-2.5 rounded-lg font-semibold hover:bg-orange-600 transition-colors">我要點餐 →</button>
        ) : isStale ? (
      ...
      {confirmClose && (
        <div className="bg-gray-50 border rounded-lg p-3 space-y-2">
          ...
          <div className="flex gap-2">
            <button onClick={() => { onClose(session.id); setConfirmClose(false); }} className="flex-1 text-sm bg-orange-500 text-white py-1.5 rounded-lg font-medium">確定關閉</button>
            <button onClick={() => setConfirmClose(false)} className="flex-1 text-sm bg-gray-100 text-gray-600 py-1.5 rounded-lg font-medium">取消</button>
          </div>
        </div>
      )}
      ...
      {confirmReset && (
        <div className="bg-red-50 border border-red-200 rounded-lg p-3 space-y-2">
          ...
          <div className="flex gap-2">
            <button onClick={() => { onReset(session.id); setConfirmReset(false); }} className="flex-1 text-sm bg-red-500 text-white py-1.5 rounded-lg font-medium">確定刪除</button>
            <button onClick={() => setConfirmReset(false)} className="flex-1 text-sm bg-gray-100 text-gray-600 py-1.5 rounded-lg font-medium">取消</button>
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      <div className="bg-white rounded-xl shadow p-6 space-y-5">
          ...
          <div className="flex flex-wrap gap-2">
            {DURATION_OPTIONS.map((d) => (
              <button key={d} type="button" onClick={() => setDuration(d)}
                className={`px-4 py-2 rounded-lg border-2 text-sm font-medium transition-colors ${duration === d ? 'border-orange-500 bg-orange-50 text-orange-600' : 'border-gray-200 text-gray-600 hover:border-orange-300'}`}
              >{d} 分鐘</button>
            ))}
        ...
        <button onClick={handleCreate} className="w-full bg-orange-500 text-white py-3 rounded-lg font-semibold hover:bg-orange-600 transition-colors">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="bg-white rounded-xl shadow p-4">
            ...
            <input type="text" placeholder="請輸入姓名" value={name}
              ...
              className="w-full border rounded-lg p-3 text-gray-800" required
            />
          </div>
          ...
          <div className="bg-white rounded-xl shadow p-4">
            ...
            {shop.menu.length === 0 ? (
              ...
              <div className="grid grid-cols-2 gap-2">
                {shop.menu.map((item) => (
                  <button type="button" key={item.id} onClick={() => handleItemSelect(item)}
                    className={`p-3 rounded-lg border-2 text-left transition-colors ${selectedItem?.id === item.id ? 'border-orange-500 bg-orange-50' : 'border-gray-200 hover:border-orange-300'}`}
                  >
                  ...
                  <div className="flex gap-2">
                    {selectedItem.sizes.map((s) => (
                      <button type="button" key={s.label} onClick={() => setSize(s.label)}
                        className={`flex-1 py-2 rounded-lg border-2 font-medium transition-colors ${size === s.label ? 'border-orange-500 bg-orange-50 text-orange-600' : 'border-gray-200 text-gray-600 hover:border-orange-300'}`}
