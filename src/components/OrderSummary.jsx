...
function SessionSummary({ session, orders, onRemoveOrder, onClose, onReset, isLeader, myName }) {
  ...
  // 非團主預設只看自己的訂單（未點餐前 myName 為空，仍顯示全部）
      ...
      <div className="flex items-center justify-between">
        ...
        {isLeader && (
          <div className="flex gap-2 flex-wrap justify-end">
            <button onClick={handleCopy} disabled={orders.length === 0}
              className="text-sm bg-orange-500 text-white px-3 py-1.5 rounded-lg font-medium hover:bg-orange-600 disabled:opacity-40"
            >{copied ? '已複製！' : '複製'}</button>
            <button onClick={() => onClose(session.id)}
              className="text-sm bg-gray-100 text-gray-700 px-3 py-1.5 rounded-lg font-medium hover:bg-gray-200"
            >關閉</button>
      ...
      {showConfirm && (
        <div className="bg-red-50 border border-red-200 rounded-xl p-4">
          ...
          <div className="flex gap-2">
            <button onClick={() => { onReset(session.id); setShowConfirm(false); }} className="flex-1 bg-red-500 text-white py-2 rounded-lg font-medium">確定清空</button>
            <button onClick={() => setShowConfirm(false)} className="flex-1 bg-gray-100 text-gray-700 py-2 rounded-lg font-medium">取消</button>
          </div>
        </div>
      )}

      {/* 統計 + 只看我的 toggle（使用者才顯示） */}
                ...
                <div className="flex items-center gap-3">
                  ...
                  {isLeader && (
                    <button onClick={() => onRemoveOrder(order.id)}
                      className="text-gray-400 hover:text-red-500 text-lg leading-none" title="刪除"
                    >×</button>
                  )}
                </div>
              </div>
      ...
      {orders.length > 0 && isLeader && (
        <div className="bg-gray-50 rounded-xl border overflow-hidden">
          <button
            ...
            className="w-full flex justify-between items-center px-4 py-3 text-sm font-semibold text-gray-500 hover:bg-gray-100"
          >
            <span>複製預覽</span>
              ...
              {isOpen && (
                <div className="border-t px-3 py-3 bg-gray-50 space-y-2">
                  ...
                  <div className="flex gap-2 pt-2">
                    <button onClick={() => handleCopy(s)} className="flex-1 bg-orange-500 text-white py-1.5 rounded-lg text-sm font-medium hover:bg-orange-600">
                      {copiedId === s.id ? '已複製！' : '複製'}
                    </button>
                    <button onClick={() => onRemoveHistory(s.id)} className="px-4 py-1.5 bg-gray-200 text-gray-600 rounded-lg text-sm hover:bg-red-100 hover:text-red-600">
                      刪除紀錄
                    </button>
                  </div>
                </div>
